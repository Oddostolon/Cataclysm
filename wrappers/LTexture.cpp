  ...
  if (!loadedSurface)
  ...
  else
    {
      ...
      mTexture = std::unique_ptr<SDL_Texture, decltype (&SDL_DestroyTexture)> (
          SDL_CreateTextureFromSurface (mRenderer.get (), loadedSurface.get ()), SDL_DestroyTexture);
...
void
LTexture::render (int x, int y, std::shared_ptr<SDL_Rect> clip /*= nullptr*/, double angle /*= 0.0*/,
