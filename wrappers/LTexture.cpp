...
void
LTexture::setRenderer (std::shared_ptr<SDL_Renderer> renderer)
{
    mRenderer = renderer;
}

bool
LTexture::loadFromRenderedText (TTF_Font *font, std::string textureText, SDL_Color textColor)
{
    free ();
    ...
    if (textSurface == nullptr)
        ...
        {
            mTexture = std::unique_ptr<SDL_Texture, decltype (&SDL_DestroyTexture)> (
                SDL_CreateTextureFromSurface (mRenderer.get (), textSurface), SDL_DestroyTexture);
            if (!mTexture)
                {
...
void
LTexture::render (int x, int y, std::shared_ptr<SDL_Rect> clip /*= nullptr*/,
                  double angle /*= 0.0*/, std::shared_ptr<SDL_Point> center /*= nullptr*/,
                  SDL_RendererFlip flip /*= SDL_FLIP_NONE*/)
{
    ...
    SDL_RenderCopyEx (mRenderer.get (), mTexture.get (), clip.get (), &renderQuad, angle,
                      center.get (), flip);
}
